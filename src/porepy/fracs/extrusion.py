...
def fractures_from_outcrop(
    pt, edges, ensure_realistic_cuts=True, family=None, **kwargs
):
    ...
    logging.info(
        "Fractures composed of " + str(split_edges.shape[0]) + "branches"
    )
    ...
    rot_ang = impose_inlcine(
        fractures, exposure, p0, frac_family=family, **kwargs
    )
    ...
    for prim, sec, p in zip(prim_frac, sec_frac, other_pt):
        ...
        if ensure_realistic_cuts and radius is not None:
            ...
            e0 = p0[:, prim]
            e1 = p1[:, prim]
            new_radius, center, _ = disc_radius_center(
                lengths[prim], e0, e1, theta=ang
            )
...
def create_fracture(center, radius, dip, strike, extra_points):
    ...
    if extra_points.shape[0] == 2:
        extra_points = np.vstack(
            (extra_points, np.zeros(extra_points.shape[1]))
        )
...
def cut_fracture_by_plane(
    ...
    **kwargs
    ...
    p[i2, 0] = (
        c[i2]
        - (n[i0] * (main_min[i0] - c[i0]) + n[i1] * (main_min[i1] - c[i1]))
        / n[i2]
    )
    ...
    p[i2, 1] = (
        c[i2]
        - (n[i0] * (main_max[i0] - c[i0]) + n[i1] * (main_min[i1] - c[i1]))
        / n[i2]
    )
    ...
    p[i2, 2] = (
        c[i2]
        - (n[i0] * (main_max[i0] - c[i0]) + n[i1] * (main_max[i1] - c[i1]))
        / n[i2]
    )
    ...
    p[i2, 3] = (
        c[i2]
        - (n[i0] * (main_min[i0] - c[i0]) + n[i1] * (main_max[i1] - c[i1]))
        / n[i2]
    )
