...
def _split_edge(vertices, edges, edge_ind, new_pt, **kwargs):
    ...
    if len(pt_ind) < 2:
        ...
        if start == pi or end == pi:
            ...
            if any(ismem):
                new_edges = np.delete(
                    new_edges, np.squeeze(np.argwhere(ismem)), axis=0
                )
        ...
        if i0 != start and i1 != end:
            ...
            if tags.size > 0:
                new_edges = np.vstack(
                    (
                        new_edges,
                        np.tile(tags[:, np.newaxis], new_edges.shape[1]),
                    )
                )
            # Combine everything.
            edges = np.hstack(
...
def remove_edge_crossings(
    vertices, edges, tol=1e-3, verbose=0, snap=True, **kwargs
):
    """
    ...
    while edge_counter < edges.shape[1]:
        ...
        tol_scaled = tol * max(
            1, np.max([np.sqrt(np.abs(c1)), np.sqrt(np.abs(c2))])
        )
        ...
        edge_counter += 1
        logger.debug(
            "Edge split into %i new parts",
            edges.shape[1] - size_before_splitting,
        )

    if verbose > 1:
        logger.info(
            "Edge intersection removal complete. Elapsed time: %g",
...
def segments_intersect_3d(start_1, end_1, start_2, end_2, tol=1e-8):
    ...
    if np.abs(discr) < tol:
        ...
        dstart_x_delta_x = (
            diff_start[1] * deltas_1[2] - diff_start[2] * deltas_1[1]
        )
        if np.abs(dstart_x_delta_x) > tol:
            return None
        dstart_x_delta_y = (
            diff_start[2] * deltas_1[0] - diff_start[0] * deltas_1[2]
        )
        if np.abs(dstart_x_delta_y) > tol:
            return None
        dstart_x_delta_z = (
            diff_start[0] * deltas_1[1] - diff_start[1] * deltas_1[0]
        )
    ...
    # or we are looking for a point intersection
        ...
        t_1 = (
            (start_2[in_discr[0]] - start_1[in_discr[0]])
            * (-deltas_2[in_discr[1]])
...
def polygon_segment_intersect(poly_1, poly_2, tol=1e-8, include_bound_pt=True):
    ...
    assert (
        np.amax(np.abs(poly_1_rot[2])) / np.amax(np.abs(poly_1_rot[:2])) < tol
    )
        ...
        for i in range(num_p2):
            ...
            if np.abs(dz) > tol:
                ...
                if inside.all():
                    ...
                    isect = np.hstack(
                        (isect, irot.dot(_to3D(isect_loc)) + center_1)
                    )
...
def is_collinear(pts, tol=1e-5):
    ...
    coll = (
        np.array(
            [
                np.linalg.norm(np.cross(p - pt0, pt1 - pt0))
                for p in pts[:, 1:-1].T
            ]
        )
        / dist
...
def dist_segment_segment_set(start, end, start_set, end_set):
    ...
    for i in range(ns):
        dl, cpi, cpj = dist_two_segments(
            start, end, start_set[:, i], end_set[:, i]
        )
        d[i] = dl
        cp[:, i] = cpi
        cp_set[:, i] = cpj
...
def dist_points_segments(p, start, end):
    ...
    if num_p < num_l:
        ...
        for ei in range(num_l):
            ...
            proj = (
                np.sum(v * line[:, ei].reshape((-1, 1)), axis=0)
                / lengths[ei] ** 2
            )
            ...
            for proj_i, bi in enumerate(np.where(between)[0]):
                d[bi, ei] = np.min(
                    dist_point_pointset(proj_p[:, proj_i], p[:, bi])
                )
...
def dist_points_polygon(p, poly, tol=1e-5):
    ...
    d_outside, p_outside = dist_points_segments(
        orig_p[:, outside_poly], start, end
    )
...
def dist_segments_polygon(start, end, poly, tol=1e-5):
    ...
    endpoint_in_polygon = np.logical_or(
        is_inside_polygon(poly_xy, start[:2]),
