...
class UpwindMixedDim(SolverMixedDim):
    ...
    def cfl(self, gb):
        deltaT = gb.apply_function(
            self.discr.cfl, self.coupling_conditions.cfl
        ).data
...
class UpwindCoupling(AbstractCoupling):
    ...
    def matrix_rhs(
        self, g_h, g_l, data_h, data_l, data_edge, d_name="discharge"
    ):
        """
        ...
        if g_h.dim == g_l.dim:
            ...
            d_00 = np.bincount(
                cells_h,
                np.sign(discharge.clip(min=0)) * discharge,
                minlength=g_h.num_cells,
            )
            d_11 = np.bincount(
                cells_l,
                np.sign(discharge.clip(max=0)) * discharge,
            ...
            diag_cc11 = np.zeros(g_l.num_cells)
            np.add.at(
                diag_cc11, cells_l, np.sign(discharge.clip(max=0)) * discharge
            )

            diag_cc00 = np.zeros(g_h.num_cells)
            np.add.at(
                diag_cc00, cells_h, np.sign(discharge.clip(min=0)) * discharge
            )
        # Compute the outflow from the higher to the lower dimensional grid
        cc[1, 0] = sps.coo_matrix(
            (-discharge.clip(min=0), (cells_l, cells_h)), shape=(dof[1], dof[0])
        )

        # Compute the inflow from the higher to the lower dimensional grid
        cc[0, 1] = sps.coo_matrix(
            (discharge.clip(max=0), (cells_h, cells_l)), shape=(dof[0], dof[1])
        )
    ...
    def cfl(self, g_h, g_l, data_h, data_l, data_edge, d_name="discharge"):
        ...
        # Since discharge is multiplied by the aperture, we get rid of it!!!!
        discharge = np.divide(
            discharge[faces_h], g_h.face_areas[faces_h] * aperture_h
        )
        # deltaT is deltaX/discharge with coefficient
