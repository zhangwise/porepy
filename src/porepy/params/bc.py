...
def face_on_side(g, side, tol=1e-8):
    ...
    for s in side:
        ...
        if s == "west" or s == "xmin":
            xm = g.nodes[0].min()
            faces.append(
                np.squeeze(np.where(np.abs(g.face_centers[0] - xm) < tol))
            )
            ...
            xm = g.nodes[1].min()
            faces.append(
                np.squeeze(np.where(np.abs(g.face_centers[1] - xm) < tol))
            )
            ...
            xm = g.nodes[2].min()
            faces.append(
                np.squeeze(np.where(np.abs(g.face_centers[2] - xm) < tol))
            )
