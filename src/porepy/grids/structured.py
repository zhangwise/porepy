...
class TensorGrid(Grid):
    ...
    def __init__(self, x, y=None, z=None, name=None):
        ...
        if y is None:
            ...
            super(TensorGrid, self).__init__(
                1, nodes, face_nodes, cell_faces, name
            )
        elif z is None:
            ...
            super(TensorGrid, self).__init__(
                3, nodes, face_nodes, cell_faces, name
            )
    ...
    def _create_1d_grid(self, nodes_x):
        ...
        cell_faces = np.vstack((face_array[:-1], face_array[1:])).ravel(
            order="F"
        )
    ...
    def _create_2d_grid(self, nodes_x, nodes_y):
        ...
        cell_faces = np.vstack(
            (face_west, face_east, face_south, face_north)
        ).ravel(order="F")
    ...
    def _create_3d_grid(self, nodes_x, nodes_y, nodes_z):
        ...
        # Cell faces
        face_x = np.arange(num_faces_x).reshape(
            num_x + 1, num_y, num_z, order="F"
        )
    ...
    def __init__(self, nx, physdims=None):
        ...
        if dims is ():  # dirty trick
            ...
            super(self.__class__, self).__init__(
                nodes_x, nodes_y, nodes_z, name=name
            )
        else:
            raise ValueError(
