...
class GmshWriter(object):
    ...
    def __write_boundary_2d(self):
        ...
        bound_line = sort_points.sort_point_pairs(
            bound_line, check_circular=True
        )
    ...
    def __write_polygons(self, boundary=False):
        ...
        """
        constants = gridding_constants.GmshConstants()
        bound_tags = self.polygon_tags.get(
            "boundary", [False] * len(self.polygons[0])
        )
        subd_tags = self.polygon_tags.get(
            "subdomain", [False] * len(self.polygons[0])
        )
