...
def simplex_grid(
    fracs=None, domain=None, network=None, subdomains=[], verbose=0, **kwargs
):
    """
...
def dfn(
    fracs, conforming, intersections=None, keep_geo=False, tol=1e-4, **kwargs
):
    ...
    if isinstance(fracs, FractureNetwork) or isinstance(
        fracs, FractureNetwork_full
    ):
        network = fracs
    else:
    ...
    if conforming:
        ...
        grids = simplex.triangle_grid_embedded(
            network, find_isect=False, **kwargs
        )
    else:
        ...
        for fi in range(len(network._fractures)):
            ...
            grids = simplex.triangle_grid(
                frac_dict, fp, verbose=False, **kwargs
            )
            ...
            assert (
                len(grids[0]) == 1
            ), "Fracture should be covered by single" "mesh"
...
def grid_list_to_grid_bucket(grids, time_tot=None, **kwargs):
    ...
    if time_tot is not None:
        logger.info(
            "Mesh construction completed. Total time "
...
def _tag_faces(grids, check_highest_dim=True):
    ...
    if check_highest_dim:
        assert len(grids[0]) == 1, "Must be exactly" "1 grid of dim: " + str(
            len(grids)
        )
    ...
    for g_h in np.atleast_1d(grids[0]):
        ...
        for g_dim in grids[1:-1]:
            for g in g_dim:
                ...
                is_tip = np.any(
                    is_tip.reshape((n_per_face, bnd_faces_l.size), order="F"),
                    axis=0,
                )
