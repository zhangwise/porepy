...
class Exporter:
    ...
    def write_pvd(self, time):
        ...
        if self.is_GridBucket:
            [
                o_file.write(
                    fm % (time[t], self._make_file_name(self.name, t, dim))
                )
                for t in time_step
        ...
        for dim in self.dims:
            ...
            for d in data:
                ...
                for i, g in enumerate(grids):
                    ...
                    if (
                        np.atleast_2d(self.gb.graph.node[g][d]).shape[1]
                        != g.num_cells
                    ):
                        raise ValueError(
                            "Field "
        ...
        if data is not None:
            for name_field, values_field in data.items():
                ...
                dataVTK = ns.numpy_to_vtk(
                    values_field.ravel(order="F"),
                    deep=True,
                    array_type=vtk.VTK_DOUBLE,
                )
                dataVTK.SetName(str(name_field))
                dataVTK.SetNumberOfComponents(
                    1 if values_field.ndim == 1 else 3
                )
    ...
    def _update_gb_VTK(self):
        if self.is_GridBucket:
            for dim in self.dims:
                ...
                self.gb_VTK[dim], self.num_elem[dim] = self._export_vtk_grid(
                    g, dim
                )
        else:
            self.gb_VTK, self.num_elem = self._export_vtk_grid(
                [self.gb], self.gb.dim
            )
    ...
    def _define_gvtk_3d(self, gs):
        ...
        for g in gs:
            ...
            num_cell_nodes = np.array(
                [nodes_per_face[i] for i in g.cell_faces.indices]
            )
...
def _point_ind(
    ...
    for ci in range(cell_ptr.size - 1):
        ...
        for fi in face_cells[loc_c]:
            ...
            delta = np.array([pts[i] - center for i in range(pts.shape[0])])[
                :, :2
            ]
        ...
        for ci in range(cell_ptr.size - 1):
            ...
            for fi in faces_cells[loc_c]:
                ...
                R = (
                    ...
                    + (
                        (1. - np.cos(angle))
                        * np.linalg.matrix_power(W, 2).ravel()
                    ).reshape((3, 3))
                )
                ##            # pts is now a npt x 3 matrix
