...
class BasicsTest(unittest.TestCase):
    ...
    def test_tpfa_coupling_2d_1d_bottom_top_dir(self):
        """
    ...
    def test_tpfa_coupling_2d_1d_left_right_dir(self):
        ...
        for g, d in gb:
            ...
            if bound_faces.size != 0:
                ...
                param.set_bc(
                    solver, bc.BoundaryCondition(g, bound_faces, labels)
                )
                param.set_bc_val(solver, bc_val)
            else:
                param.set_bc(
                    "flow", bc.BoundaryCondition(g, np.empty(0), np.empty(0))
                )
            d["param"] = param
            d["param"] = param
...
def matrix_rhs_pressure_for_test_tpfa_coupling_3d_2d_1d_0d():
    A = np.array(
        [
            [
                3.99994,
