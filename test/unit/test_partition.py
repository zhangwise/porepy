...
class TestConnectivityChecker(unittest.TestCase):
    ...
    def test_not_connected(self):
        ...
        assert np.array_equal(
            np.sort(p_sub[components[0]]), np.array([0, 1, 4, 5])
        )
        assert np.array_equal(
            np.sort(p_sub[components[1]]), np.array([10, 11, 14, 15])
        )
    ...
    def test_subgrid_connected(self):
        ...
        assert is_connected
        assert np.array_equal(
            np.sort(components[0]), np.arange(sub_g.num_cells)
        )
...
class TestPartitionGrid(unittest.TestCase):
    def test_identity_partitioning(self):
        ...
        sub_g, face_map_list, node_map_list = pp.partition.partition_grid(
            g, ind
        )
    ...
    def test_single_cell_partitioning(self):
        ...
        for ci, (sg, fm, nm) in enumerate(
            zip(sub_g, face_map_list, node_map_list)
        ):
            ...
            n_known = np.where(
                g.face_nodes.todense()[:, f_known].sum(axis=1) > 0
            )[0]
