    ...
    **kwargs
    ...
    if max_memory is None:
        ...
        for p in range(part.max()):
            ...
            loc_flux, loc_bound_flux, loc_faces = mpfa_partial(
...
def mpfa_partial(
    ...
    flux_loc, bound_flux_loc = _mpfa_local(
...
def _block_diagonal_structure(
    sub_cell_index, cell_node_blocks, nno, bound_exclusion
):
    ...
    rows2blk_diag = sps.coo_matrix(
        (
            np.ones(sorted_nodes_rows.size),
...
def _create_bound_rhs(
    bnd, bound_exclusion, subcell_topology, sgn, g, num_flux, num_pr
):
    """
    ...
    if neu_ind.size > 0 and dir_ind.size > 0:
        ...
        raise ValueError(
            "Boundary values should be either Dirichlet or " "Neumann"
        )
