...
def main(id_problem, tol=1e-5, N_pts=1000, if_export=False):
    ...
    mesh_size = 0.025  # 0.05 0.01
    folder_export = (
        "example_2_3_vem_" + str(mesh_size) + "/" + str(id_problem) + "/"
    )
    file_export = "vem"

    gb = example_2_3_create_grid.create(
        id_problem, is_coarse=False, mesh_size=mesh_size, tol=tol
    )
    ...
    num_cells = np.array(gb.num_cells(lambda g: g.dim == 2))
