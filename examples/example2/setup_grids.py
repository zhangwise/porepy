...
def perturb(g, rate, dx):
    rand = np.random.rand(g.dim, g.num_nodes)
    g.nodes[: g.dim] += rate * dx * (rand - 0.5)
    return g
...
def grid_sequence(
    basedim, num_levels, grid_type, pert=0, ref_rate=2, domain=None
):
    dim = basedim.shape[0]
