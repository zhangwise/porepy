...
def dfm_3d_from_csv(
    file_name, tol=1e-4, elliptic_fractures=False, **mesh_kwargs
):
    """
...
def elliptic_network_3d_from_csv(
    file_name, has_domain=True, tol=1e-4, degrees=False
):
...
def lines_from_csv(
    f_name, tagcols=None, tol=1e-8, max_num_fracs=None, **kwargs
):
    ...
    with open(f_name, "r") as f:
        for line in f:
            if line.strip() == "BEGIN FORMAT":
                ...
                # Read tess_fractures
                tess_fracs, tess_frac_ids, tess_sgn = read_fractures(
                    f, is_tess=True
                )
...
def read_dfn_grid(folder, num_fractures, case_id, **kwargs):
    ...
    for f_id in np.arange(num_fractures):
        ...
        g_2d[f_id].global_point_ind = (
            np.arange(g_2d[f_id].num_nodes) + global_node_id
        )
...
def _dfn_grid_2d(folder, post, **kwargs):
    ...
    with open(face_file_name, "r") as f:
        ...
        face_nodes_indptr = np.hstack(
            (np.arange(0, 2 * num_faces, 2), 2 * num_faces)
        )
