...
def snap_fracture_set_2d(
    pts, edges, snap_tol, termination_tol=1e-2, max_iter=100
):
    ...
    while counter < max_iter:
        ...
        logger.debug(
            "Iteration " + str(counter) + ", max difference" + str(diff)
        )
        pts = pn
    ...
    if counter < max_iter:
        ...
        logger.info(
            "Maximum modification " + str(np.max(np.abs(pts - pts_orig)))
        )
        return pts, True
    else:
