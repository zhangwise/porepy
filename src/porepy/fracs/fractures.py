...
class Fracture(object):
    ...
    def add_points(
        self, p, check_convexity=True, tol=1e-4, enforce_pt_tol=None
    ):
        """
    ...
    def intersects(self, other, tol, check_point_contact=True):
        ...
        scaled_tol = tol * max(
            1, max(np.max(np.abs(s_2_o)), np.max(np.abs(o_2_s)))
        )
        ...
        if int_points.shape[1] > 1:
            int_points, _, _ = setmembership.unique_columns_tol(
                int_points, tol=tol
            )
        ...
        def point_on_segment(ip, poly, need_two=True):
            ...
            if (
                need_two
                and ip.shape[1] < 2
                or ((not need_two) and ip.shape[1] < 1)
            ):
                return False
            start = poly
    ...
    def impose_boundary(self, box, tol):
        ...
        box_array = np.array(
            [[x0_box, x1_box], [y0_box, y1_box], [z0_box, z1_box]]
        )
        ...
        def outside_box(p, bound_i):
            ...
            outside = np.vstack(
                (west_of, east_of, south_of, north_of, beneath, above)
            )
...
class Intersection(object):
    ...
    def __init__(
        self, first, second, coord, bound_first=False, bound_second=False
    ):
...
class FractureNetwork(object):
    ...
    def find_intersections(self, use_orig_points=False):
        ...
        for i, first in enumerate(self._fractures):
            for j in range(i + 1, len(self._fractures)):
                ...
                isect, bound_first, bound_second = first.intersects(
                    second, self.tol
                )
    ...
    def split_intersections(self):
        ...
        logger.info(
            "Finished fracture splitting after %.5f seconds",
            time.time() - start_time,
        )
    ...
    def _uniquify_points_and_edges(
        self, all_p, edges, edges_2_frac, is_boundary_edge
    ):
        ...
        for old_i, new_i in enumerate(all_2_unique_e):
            ...
            tmp = np.hstack(
                (is_boundary_edge_new[new_i], is_boundary_edge[old_i])
            )
        ...
        edges_2_frac = [
            np.unique(np.array(edges_2_frac[i]))
            for i in range(len(edges_2_frac))
        ]
    ...
    def _remove_edge_intersections(
        ...
        for fi, frac in enumerate(self._fractures):
            ...
            edges_loc = np.vstack(
                (edges[:, edges_loc_ind], np.array(edges_loc_ind))
            )
    ...
    def fractures_of_points(self, pts):
        ...
        for i in pts:
            ...
            for poly_ind, poly in enumerate(self.decomposition["polygons"]):
                ...
                if any(ismem):
                    fracs_loc.append(
                        self.decomposition["polygon_frac"][poly_ind]
                    )
    ...
    def add_subdomain_boundaries(self, vertexes):
        ...
        Parameters:
            ...
            np.array([[x0, x1, x2, x3],
                      ...
                      [z0, z1, z2, z3]])
        """
        subdomain_tags = self.tags.get(
            "subdomain", [False] * len(self._fractures)
        )
        for f in vertexes:
    ...
    def _make_bounding_planes(self, box, keep_box=True):
        ...
        # Collect in a list to allow iteration
        bound_planes = [west, east, south, north, bottom, top]
        boundary_tags = self.tags.get(
            "boundary", [False] * len(self._fractures)
        )
    ...
    def _determine_mesh_size(self, **kwargs):
        ...
        logger.info(
            "Minimal distance between points encountered is "
            + str(np.min(dist))
        )
        mesh_size_min = np.maximum(
            mesh_size_dist, self.mesh_size_min * np.ones(num_pts)
        )
        mesh_size = np.minimum(
            mesh_size_min, self.mesh_size_frac * np.ones(num_pts)
        )
        ...
        if (self.mesh_size_bound is not None) and (on_boundary is not None):
            ...
            mesh_size[on_boundary] = np.maximum(mesh_size, mesh_size_bound)[
                on_boundary
            ]
        return mesh_size
        ...
        for fi, f in enumerate(self._fractures):
            ...
            for of in self._fractures:
                ...
                for si, fs in enumerate(f.segments()):
                    ...
                    d, cp_f, _ = cg.dist_segment_segment_set(
    ...
    def to_gmsh(self, file_name, in_3d=True, **kwargs):
        ...
        intersection_points = intersection_points[
            np.logical_not(int_pts_on_boundary)
        ]
