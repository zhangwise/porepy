...
class FlowData(EllipticDataAssigner):
    ...
    def permeability(self):
        kxx = np.ones(self.grid().num_cells) * np.power(
            1e4, self.grid().dim < 2
        )
    ...
    def bc(self):
        ...
        if (
            self.grid().dim < 1
            or abs(self.grid().face_centers[0, 0] - .5) < 1e-3
        ):
    ...
    def bc_val(self):
        ...
        if (
            self.grid().dim > 0
            and abs(self.grid().face_centers[0, 0] - .5) > 1e-3
        ):
    ...
    for g, d in gb:
        ...
        if g.dim == 0:
            perm = tensor.SecondOrderTensor(
                3, k_intersection * np.ones(g.num_cells)
            )
...
def barplot(values, title="Schur complement", plot_type="error_"):
    ...
    plt.yticks(
        yedges,
        ["1e%.0d" % np.log10(k) for k in vertical_permeabilities],
        size=sz_small,
    )
    sz = 14
