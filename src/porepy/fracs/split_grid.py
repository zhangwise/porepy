...
def update_face_cells(face_cells, face_id, i):
    ...
    for j, f_c in enumerate(face_cells):
        ...
        if j == i:
            ...
            new_indptr = f_c.indptr[-1] * np.ones(
                face_id.size, dtype=f_c.indptr.dtype
            )
    ...
    for node in nodes:
        ...
        for j in range(colors.size):
            ...
            if offset > 0 and colors.size > 1:
                new_nodes[:, j] -= (
                    avg_normal(g, faces_of_node_t[is_colored]) * offset
                )
        ...
        g.face_nodes.indptr = np.insert(
            g.face_nodes.indptr, t_node + 1, face_pos[1:-1]
        )
        g.face_nodes._shape = (
        ...
        g.nodes = np.insert(
            g.nodes, [t_node] * new_nodes.shape[1], new_nodes, axis=1
        )
