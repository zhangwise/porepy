...
class EllipticModel:
    ...
    def discharge(self, discharge_name="discharge"):
        if self.is_GridBucket:
            ...
            fvutils.compute_discharges(
                self.grid(), self.physics, self.pressure_name, self._data
            )
    ...
    def permeability(self, perm_names=["kxx", "kyy", "kzz"]):
        ...
        for n in perm_names:
            ...
            if self.is_GridBucket:
                ...
                self._data[n] = (
                    self._data["param"].get_permeability().perm[ind, ind, :]
                )
    ...
    def save(self, variables=None, save_every=None):
        if variables is None:
            ...
            if not self.is_GridBucket:
                variables = {
                    k: self._data[k] for k in variables if k in self._data
                }
