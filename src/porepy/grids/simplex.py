...
class TriangleGrid(Grid):
    ...
    def __init__(self, p, tri=None, name=None):
        ...
        indptr = np.hstack(
            (
                np.arange(
                    0, num_nodes_per_face * num_faces, num_nodes_per_face
                ),
        ...
        cell_faces = cell_faces.reshape(num_faces_per_cell, num_cells).ravel(
            "F"
        )
        indptr = np.hstack(
            (
                np.arange(
                    0, num_faces_per_cell * num_cells, num_faces_per_cell
                ),
        ...
        super(TriangleGrid, self).__init__(
            2, nodes, face_nodes, cell_faces, name
        )
    ...
    def cell_node_matrix(self):
        ...
        # Consistency check
        assert np.all(
            accumarray.accum(col, np.ones(col.size)) == (self.dim + 1)
        )
...
class StructuredTriangleGrid(TriangleGrid):
    ...
    def __init__(self, nx, physdims=None):
        ...
        tri_base = np.vstack(
            (ind_1, ind_2, ind_3, ind_1, ind_3, ind_4)
        ).reshape((3, -1), order="F")
        ...
        super(self.__class__, self).__init__(
            p, tri, name="StructuredTriangleGrid"
        )
