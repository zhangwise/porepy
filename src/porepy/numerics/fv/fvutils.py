...
class SubcellTopology(object):
    ...
    def pair_over_subfaces(self, other):
        ...
        pair_over_subfaces = sps.coo_matrix(
            (sgn[0], (self.subfno, self.subhfno))
        )
...
def compute_dist_face_cell(g, subcell_topology, eta):
    ...
    cols += matrix_compression.rldecode(
...
def invert_diagonal_blocks(mat, s, method=None):
    ...
    def invert_diagonal_blocks_numba(a, size):
        ...
        def inv_python(indptr, ind, data, sz):
            ...
            row_cols_start_ind = np.zeros(
                num_cols_per_row.size + 1, dtype=np.int32
            )
            ...
            for iter1 in range(sz.size):
                ...
                for iter2 in range(n):  # Local rows
                    ...
                    for _ in range(
                        ...
                        loc_col = (
                            ind[data_counter] - block_row_starts_ind[iter1]
                        )
...
class ExcludeBoundaries(object):
    ...
    def exclude_dirichlet(self, other):
        ...
        if self.bc_type == "scalar":
            ...
            if self.nd == 3:
                exclude_dir = np.append(
                    exclude_dir, [self.exclude_dir_z * other]
                )
    ...
    def exclude_neumann(self, other):
        ...
        if self.bc_type == "scalar":
            ...
            if self.nd == 3:
                exclude_neu = np.append(
                    exclude_neu, [self.exclude_neu_z * other]
                )
...
def cell_ind_for_partial_update(g, cells=None, faces=None, nodes=None):
    ...
    if cells is not None:
        ...
        # above.
        active_vertexes[
            np.squeeze(np.where(g.face_nodes * active_faces > 0))
        ] = 1

        # Finally, get hold of all cells that shares one of the secondary
        # vertexes.
        cells_overlap = np.squeeze(
            np.where((cn.transpose() * active_vertexes) > 0)
        )
    ...
    if faces is not None:
        ...
        primary_vertex[
            np.squeeze(np.where((g.face_nodes * primary_faces) > 0))
        ] = 1
        active_face_ind = np.squeeze(
    ...
    if nodes is not None:
        ...
        num_active_face_nodes = np.array(
            g.face_nodes.transpose() * active_vertexes
        )
...
def compute_discharges(
    gb, physics="flow", d_name="discharge", p_name="pressure", data=None
):
    """
    ...
    if not isinstance(gb, GridBucket):
        ...
        if data.get("flux") is not None:
            dis = data["flux"] * data[p_name] + data[
                "bound_flux"
    ...
    for g, d in gb:
        if g.dim > 0:
            ...
            if d.get("flux") is not None:
                dis = d["flux"] * d[p_name] + d["bound_flux"] * pa.get_bc_val(
                    physics
                )
            else:
                dis = np.zeros(g.num_faces)
