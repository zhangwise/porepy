...
def tetrahedral_grid(
    fracs=None, box=None, network=None, subdomains=[], **kwargs
):
    """
    ...
    **kwargs: To be explored.
    ...
    if mesh_size_frac is not None and mesh_size_min is not None:
        ...
        network.insert_auxiliary_points(
            mesh_size_frac, mesh_size_min, mesh_size_bound
        )
    ...
    gmsh_status = gmsh_interface.run_gmsh(
        in_file, out_file, dims=3, **gmsh_opts
    )
...
def triangle_grid_run_gmsh(file_name, **kwargs):
    ...
    # Run gmsh
    gmsh_status = gmsh_interface.run_gmsh(
        in_file, out_file, dims=2, **gmsh_opts
    )
