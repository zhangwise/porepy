...
def generate_coarse_grid_single(g, subdiv, face_map):
    ...
    for cellId, cell in enumerate(cells_list):
        ...
        cell_centers[:, cellId] = np.average(
            g.cell_centers[:, cells_old], axis=1
        )
    ...
    nodes = np.array(
        [nodes_id[np.where(nodes_list == n)[0]] for n in nodes]
    ).ravel()
...
def create_partition(A, seeds=None, **kwargs):
    ...
    vals = sps.csr_matrix(
        accumarray.accum(
            candidates, connection.data[connection_idx], size=[Nc, NC]
        )
    )
    ...
    while np.any(not_found):
        ...
        if len(nc) > 0:
            vals += sps.csr_matrix(
                (nv, (nc, np.repeat(mi, len(nc)))), shape=(Nc, NC)
            )
