...
from porepy.numerics.darcy_and_transport import (
    DarcyAndTransport,
...
def boundary_face_type(g):
    ...
    if g.dim < 2:
        return np.array([])
    if g.dim == 2 and not np.isclose(
        g.cell_centers[0, 0], g.cell_centers[0, 1]
    ):
        return np.array([])
...
class DarcySolver(EllipticModel):
    ...
    def solve(self, max_direct=40000, callback=False, **kwargs):
        ...
        if self.el:
            to_be_eliminated = SC.dofs_of_dimension(
                other.grid(), other.lhs, eldim
            )
