...
class MainTester(unittest.TestCase):
    ...
    def solve_system_homogeneous_perm(
        ...
        u_bound[bound_faces] = an_sol.u_f(
            xf[0, bound_faces], xf[1, bound_faces]
        )
        # Right hand side
        xc = g.cell_centers
        b = (
            an_sol.rhs_f(xc[0], xc[1]) * g.cell_volumes
    ...
    def solve_system_homogeneous_elasticity(
        ...
        rhs = (
            np.vstack(
                (an_sol.rhs_x_f(xc[0], xc[1]), an_sol.rhs_y_f(xc[0], xc[1]))
            )
            * g.cell_volumes
        )
    ...
    def solve_system_chi_type_elasticity(
        self, g, bound_cond, an_sol, chi, kappa
    ):
        char_func_cells = chi(g.cell_centers[0], g.cell_centers[1]) * 1.
...
class CartGrid2D(MainTester):
    ...
    def setUp(self):
        ...
        def chi(xcoord, ycoord):
            return np.logical_and(
                np.greater(xcoord, 0.5), np.greater(ycoord, 0.5)
            )
    ...
    def test_homogeneous_mpfa(self):
        ...
        perm = 1
        k = tensor.SecondOrderTensor(
            2, perm * np.ones(self.g_nolines.num_cells)
        )
