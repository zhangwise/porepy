...
class Grid(object):
    ...
    def __compute_geometry_3d(self):
        ...
        edge_2_face = sps.coo_matrix(
            (
                np.ones(num_face_nodes),
                (np.arange(num_face_nodes), face_node_ind),
            )
        ).tocsc()

        # Define temporary face center as the mean of the face nodes
        tmp_face_center = (
            self.nodes[:, face_nodes] * edge_2_face / num_nodes_per_face
        )
        ...
        along_edge = (
            self.nodes[:, face_nodes[next_node]] - self.nodes[:, face_nodes]
        )
        ...
        def bincount_nd(arr, weights):
            ...
            for iter1 in range(dim):
                count[iter1] = np.bincount(
                    arr, weights=weights[iter1], minlength=sz
                )
            return count
        ...
        tet_volumes = (
            np.sum(dist_cellcenter_subface * outer_normals, axis=0) / 3
        )
        ...
        rel_centroid = (
            bincount_nd(cell_numbers, tet_volumes * tri_centroids)
            / cell_volumes
        )
    ...
    def cell_diameters(self, cn=None):
        ...
        def diam(n):
            return np.amax(
                np.linalg.norm(
                    self.nodes[:, n[0, :]] - self.nodes[:, n[1, :]], axis=0
                )
            )
