...
class FracturedMpsa(Mpsa):
    ...
    def discretize_fractures(self, g, data, faces=None, **kwargs):
        ...
        internal_stress = sps.hstack(
            (
                sgn_left * stress[int_b_left, :]
                + sgn_right * stress[int_b_right, :],
                frac_stress_diff,
            )
        )

        A = sps.vstack(
            (sps.hstack((a, b_internal)), internal_stress), format="csr"
        )
...
def mpsa(g, constit, bound, eta=None, inverter=None, max_memory=None, **kwargs):
    ...
    if max_memory is None:
        ...
        logger.info(
            "Split MPSA discretization into " + str(num_part) + " parts"
        )
...
def mpsa_partial(
    ...
    outside = np.setdiff1d(
        np.arange(g.num_faces), active_faces, assume_unique=True
    )
...
def _mpsa_local(g, constit, bound, eta=0, inverter="numba"):
    ...
    if bound_exclusion.bc_type == "scalar":
        rhs_bound = create_bound_rhs(
            bound, bound_exclusion, subcell_topology, g
        )
    elif bound_exclusion.bc_type == "vectorial":
        rhs_bound = create_bound_rhs_nd(
            bound, bound_exclusion, subcell_topology, g
        )
...
def _tensor_vector_prod(g, constit, subcell_topology):
    ...
    normals = (
        g.face_normals[:, subcell_topology.fno]
    ...
    node_vol = (
        np.bincount(
            subcell_topology.nno, weights=cell_vol[subcell_topology.cno]
        )
        / g.dim
    )
    ...
    for iter1 in range(nd):
        ...
        csym_mat = sps.csr_matrix(
            (sym_vals.ravel("C"), cc.ravel("F"), ind_ptr_c)
        )
        casym_mat = sps.csr_matrix(
            (asym_vals.ravel("C"), cc.ravel("F"), ind_ptr_c)
        )
...
def _block_diagonal_structure(
    ...
    if bound_exclusion.bc_type == "scalar":
        node_occ = np.hstack(
            (np.tile(nno_stress, nd), np.tile(nno_displacement, nd))
        )
...
def create_bound_rhs(bound, bound_exclusion, subcell_topology, g):
    ...
    is_neu = bound_exclusion.exclude_dirichlet(
        bound.is_neu[fno].astype("int64")
    )
...
def create_bound_rhs_nd(bound, bound_exclusion, subcell_topology, g):
    ...
    num_stress = (
        bound_exclusion.exclude_dir_x.shape[0]
        + bound_exclusion.exclude_dir_y.shape[0]
    )
    num_displ = (
        bound_exclusion.exclude_neu_x.shape[0]
        + bound_exclusion.exclude_neu_y.shape[0]
    )
    if nd == 3:
    ...
    # We also need to account for all half faces, that is, do not exclude
    # Dirichlet and Neumann boundaries.
    neu_ind_single_all_x = np.argwhere(
        bound.is_neu[0, fno].astype("int")
    ).ravel("F")
    neu_ind_single_all_y = np.argwhere(
        bound.is_neu[1, fno].astype("int")
    ).ravel("F")
    ...
    if nd == 3:
        ...
        neu_ind_single_all_z = np.argwhere(
            bound.is_neu[2, fno].astype("int")
        ).ravel("F")
    ...
    if nd == 3:
        ind_is_bnd_neu_z = np.argwhere(np.isin(is_bnd_neu, is_bnd_neu_z)).ravel(
            "F"
        )
    ...
    dir_ind_single_all_x = np.argwhere(
        bound.is_dir[0, fno].astype("int")
    ).ravel("F")
    dir_ind_single_all_y = np.argwhere(
        bound.is_dir[1, fno].astype("int")
    ).ravel("F")
    ...
    if nd == 3:
        ...
        dir_ind_single_all_z = np.argwhere(
            bound.is_dir[2, fno].astype("int")
        ).ravel("F")
    ...
    if nd == 3:
        ind_is_bnd_dir_z = np.argwhere(np.isin(is_bnd_dir, is_bnd_dir_z)).ravel(
            "F"
        )
...
def __rearange_columns_displacement_eqs(
    d_cont_grad, d_cont_cell, num_sub_cells, nd
):
    ...
    # sorting indices
    d_cont_grad_map = np.argsort(
        np.tile(rep_ci_single_blk, nd), kind="mergesort"
    )
