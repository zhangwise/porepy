...
def process_intersections(
    ...
    for frac_ind, frac in enumerate(grids):
        ...
        for ind_1d, g_1d in enumerate(frac[1]):
            ...
            g_in_other = np.where(intersections[other_frac_ind] == frac_ind)[0][
                0
            ]
...
def combine_grids(g, g_1d, h, h_1d, global_ind_offset, list_of_grids, tol):
    ...
    new_faces = update_face_nodes(
        g, delete_faces, num_new_faces, new_nodes_offset
    )
    ...
    new_nodes_offset = new_nodes[0]
    new_faces = update_face_nodes(
        h, delete_faces, num_new_faces, new_nodes_offset
    )
...
def update_nodes(
    ...
    new_nodes = (
        num_nodes_orig - delete_nodes.size + np.arange(new_grid_1d.num_nodes)
    )
...
def update_face_nodes(
    ...
    new_face_nodes = np.tile(
        np.arange(num_new_faces), (nodes_per_face, 1)
...
def update_cell_faces(
    ...
    new_sgn = [
        g.cell_faces.data[indptr[i] : indptr[i + 1]] for i in range(g.num_cells)
    ]
...
def update_face_tags(g, delete_faces, new_faces):
    ...
    for key in keys:
        if hasattr(g, "tags"):
            ...
            num_new = np.array(
                [len(new_faces[i]) for i in range(len(new_faces))]
            )
