...
#------------------------------------------------------------------------------#
    ...
    def test_tag_2d_simplex(self):
        ...
        assert np.array_equal(g.tags['tip_nodes'], [False]*g.num_nodes)
        known = \
        np.array([ True,  True, False,  True, False, False,  True, False, False,
                   True, False,  True, False, False, False, False, False, False,
                  False,  True, False,  True, False, False, False, False, False,
                  False, False,  True,  True,  True,  True], dtype=bool)
        assert np.array_equal(g.tags['domain_boundary_faces'], known)
        known = \
        np.array([ True,  True,  True,  True,  True, False, False,  True,  True,
        ...
        for g, _ in gb:
            ...
            if g.dim == 1:
                assert np.array_equal(g.tags['fracture_faces'],
                                      [False]*g.num_faces)
                assert np.array_equal(g.tags['fracture_nodes'],
                                      [False]*g.num_nodes)
                assert np.array_equal(g.tags['tip_faces'], [False]*g.num_faces)
                assert np.array_equal(g.tags['tip_nodes'], [False]*g.num_nodes)
                known = [0, 4]
            ...
            if g.dim == 2:
                ...
                assert np.array_equal(g.tags['tip_faces'], [False]*g.num_faces)
                assert np.array_equal(g.tags['tip_nodes'], [False]*g.num_nodes)
                known = [0, 4, 5, 9, 10, 14, 15, 19, 20, 21, 22, 23, 36, 37,\
                         38, 39]
                computed = np.where(g.tags['domain_boundary_faces'])[0]
                assert np.array_equal(computed, known)
                known = [0, 1, 2, 3, 4, 5, 9, 10, 11, 18, 19, 20, 24, 25, 26, \
                         27, 28, 29]
                computed = np.where(g.tags['domain_boundary_nodes'])[0]
                assert np.array_equal(computed, known)
    ...
    def test_tag_2d_1d_cart_complex(self):
        ...
        for g, _ in gb:
            ...
            if g.dim == 2:
                ...
                computed = np.where(g.tags['domain_boundary_faces'])[0]
                assert np.array_equal(computed, known)
                known = [0, 1, 2, 3, 4, 5, 9, 10, 11, 20, 21, 22, 26, 27, 28, \
                         29, 30, 31]
                computed = np.where(g.tags['domain_boundary_nodes'])[0]
                assert np.array_equal(computed, known)
