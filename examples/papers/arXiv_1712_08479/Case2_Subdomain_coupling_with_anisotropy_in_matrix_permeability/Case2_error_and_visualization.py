...
for i, n in enumerate(ny):
    ...
    for j, y in enumerate(yfactor):
        for k, d in enumerate(deg):
            ...
            p_eq = read_csv(
                equi_folder + "/pressures" + appendix + ".csv", nc_ref
            )
            ...
            t_red = read_csv(
                reduced_folder + "/tracer" + appendix_coarse + ".csv", nc
            )[reordering]
...
plt.semilogy(
    [1, 2], [np.power(1 / 2, i) * 3.5e-2 for i in [1, 2]], c="k", ls=ls[-1]
)
plt.xticks(np.arange(len(ny)), ny)
...
plt.ylabel(r"$E$", rotation=0)
plt.semilogy(
    [1, 2], [np.power(1 / 2, i) * 1.2e-1 for i in [1, 2]], c="k", ls=ls[-1]
)
